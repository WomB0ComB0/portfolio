...
install_nix() {
    case "$(uname -s)" in
        Linux|Darwin)
            log_info "Installing Nix package manager (Determinate Systems installer with flakes support)..."
            if [[ $YES -eq 1 ]]; then
            ...
            log_success "Nix installed with flakes support. Restart your shell or run '. /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh'."
            ;;
        MINGW*|CYGWIN*|MSYS*)
            log_warn "Nix is not officially supported on Windows. Consider using WSL2."
            log_info "See: https://nixos.org/manual/nix/stable/installation/installing-binary.html#windows-wsl"
            ;;
        *)
            log_error "Unsupported OS for Nix: $(uname -s)"
            exit 1
            ;;
    esac
}
...
if ! command_exists nix; then
    if prompt "Nix is not installed. Install it?"; then
        install_nix
    else
        log_warn "Nix is optional but recommended for reproducible development environments."
    fi
else
    log_info "Nix is already installed: $(nix --version 2>/dev/null || echo 'unknown version')"
fi
